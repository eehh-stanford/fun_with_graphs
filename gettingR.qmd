...
`R` is the software. RStudio is what is known as an Integrated Development Environment (IDE). Basically, an IDE is a way of running base software the provides various tools to make the coding experience easier (e.g., an editor, debugger, and interactive graphics facilities).

Here is a brief video explaining what to do:

{{< video https://youtu.be/m5BZkW54LI8 >}}

What follows are some old notes that are very Base-`R` focused. The thing is, even though most people use the tidyverse these days, it's still valuable to understand Base-`R` (since that's what everything else is built upon!)
...
require(igraph)
rhesus <- read.table("~/Teaching/social_networks/data/sade1.txt", skip=1, header=FALSE)
...
(kids <- read.table("~/Teaching/social_networks/data/strayer_strayer1976-fig2.txt", header=FALSE))
```
...
```{r}
quercus <- read.delim("~/Teaching/social_networks/data/quercus.txt", skip=24, sep="\t", header=TRUE)
head(quercus)
```
...
```{r}
donner <- read.table("~/Teaching/social_networks/data/donner.dat", header=TRUE, skip=2)
...
sextable <- read.csv("~/Teaching/social_networks/data/nhsls_sextable.txt", header=FALSE)
